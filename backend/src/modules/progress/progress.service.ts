...
export class ProgressService {
  ...
  async markTopicComplete(userId: string, courseId: string, topicId: string, topicTitle: string): Promise<Progress> {
    let progress = await this.repo.findOne({ where: { userId, courseId, topicId } });
    if (!progress) {
  ...
  async unmarkTopicComplete(userId: string, courseId: string, topicId: string): Promise<Progress | null> {
    const progress = await this.repo.findOne({ where: { userId, courseId, topicId } });
  ...
  async getSemesterProgress(userId: string, courseIds: string[]): Promise<object> {
    ...
    for (const p of allProgress) {
      if (!byCourse[p.courseId]) byCourse[p.courseId] = { completed: 0, total: 0 };
